...
export default class ControllerCategoria{
    ...
    excluir(req, res){
        ...
        if(req.method === "DELETE"){
            ...
            if (codigoCategoria) {
                ...
                categoria.excluir(codigoCategoria)
                    .then(() => {
                        res.status(200).json({
                            "status": true,
    ...
    buscar(req, res){
        ...
        if(req.method === "GET" && req.is("application/json")){
            const codigoCategoria = req.params.codigoCategoria; // Suponha que o código da categoria a ser buscada seja passado como parâmetro na URL.
    
            if (codigoCategoria) {
                Categoria.buscar(codigoCategoria)
                    .then((categoria) => {
                        if (categoria) {
                            res.status(200).json({
                                "status": true,
                ...
                res.status(400).json({
                    "status": false,
                    "mensagem": "Por gentileza, informe o código da categoria a ser buscada na URL"
                });
            }
        } else {
            res.status(400).json({
